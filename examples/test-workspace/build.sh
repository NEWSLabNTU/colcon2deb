...
# Script to build test workspace Debian packages
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_DIR="${SCRIPT_DIR}/source"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

echo "=== Building Test Workspace Debian packages ==="

# Check if source directory exists
if [ ! -d "${SOURCE_DIR}" ]; then
    echo "Error: Source directory not found at ${SOURCE_DIR}"
    echo "This is unexpected - the test workspace should be pre-configured."
    exit 1
fi
...
if [ "${PACKAGE_COUNT}" -eq 0 ]; then
    ...
    echo "This is unexpected - the test workspace should have pre-configured packages."
    exit 1
fi

echo "Found ${PACKAGE_COUNT} ROS packages in test workspace"
...
echo "=== Build complete ==="
echo "Debian packages are available in: ${SCRIPT_DIR}/build/dist/"
